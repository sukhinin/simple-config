...
internal class ConfigTest : ShouldSpec({
    ...
    should("parse list values") {
        val config = SimpleConfig(mapOf("key1" to "", "key2" to ",", "key3" to "value1, value2 ,"))
    ...
    should("scope configs by label to map") {
        val config = SimpleConfig(mapOf("key1.key11" to "value1", "key1.key12" to "value2", "key2.key21" to "value3"))
    ...
    should("dump config to string") {
        val config = SimpleConfig(mapOf("key1" to "value1", "key2" to "value2"))
